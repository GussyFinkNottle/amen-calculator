...
\title{AMEN: arithmetical calculator}
...
\section{The Naperian combinators: $+$, $\times$, $\wedge$, $0$.}
...
Instead of |naught|, I shall use the operator |(<>)|, that looks
a little like a `|0|'. As an infix operator, it throws away its left argument, and returns its right. 
\begin{code}
(<>) = naught
zero = naught
one  = zero ^ zero
...
The definitions above generate an equivalence relation between
(possibly open) terms in a signature: the least equivalence relation
...
I shall call this relation $\zeta$-equality.  All equations 
...
different `cons' operations, each with an distinct arithmetical flavour.
...
if one exists.
Such a thing lets us see the value of an expression, but not how it was
arrived at.
...
with arithmetical combinators. It provides the basic means of interchanging the positions
of variables: | a^b = b^(a^) = b^a^(^) |. 
...
-- token parsing
paren p = ((lit Lpar `grdl` p) `grdr` lit Lpar )
...
tokens (inp@(c:cs)) | is_digit c
          = id_t (vd c) cs where
                ...
                id_t b (c:cs)               = Id (b []): tokens inp
                vd = digitToInt
-}

-- optspace = rep (sat is_space)
...
{-
variable = grdl optspace (
           Parser (\s-> [ (V (c:cs),s'') 
                        | (c,s') <- prun (sat is_alphabetic) s
                        , (cs,s'') <- prun (rep (sat is_idchar)) s' ]))
constant = grdl optspace (
           Parser (\s-> [ (V cs,s') 
                        | (cs,s') <- prun (rep1 (sat is_digit) ) s  ]))
-}

{-
atomic         = variable `alt` constant `alt` paren expression
additive       = Parser (\s-> [ (fo (:+:) x xs ,s') | ((x:xs),s') <- prun (repsep multiplicative (lit '+')) s ])
multiplicative = Parser (\s-> [ (fo (:*:) x xs ,s') | ((x:xs),s') <- prun (repsep exponential (lit '*')) s ])
exponential    = Parser (\s-> [ (fo (:^:) x xs ,s') | ((x:xs),s') <- prun (repsep atomic (lit '^')) s ])
-- do  (x:xs) <- repsep multiplicative (lit '+') ; return (fo (:+:) x xs) 
-- multiplicative = do  (x:xs) <- repsep exponential    (lit '*') ; return (fo (:*:) x xs) 
-- exponential    = do  (x:xs) <- repsep atomic         (lit '^') ; return (fo (:^:) x xs) 
expression     = additive
...
instance Read E where
  readsPrec d r = prun expression r 
-}
\end{code}


\subsection{For a talk in Chamb{\'e}ry}
\begin{code}
demoMul     = let d = va :+: vb in d
demoOne     = let d = V"0" in d 
demoExp     = let d = (va :^: cPair) :*: (vb :^: V"^") in d
demoExp'    = let d = (va :*: cPair) :+: (vb :*: V"^") in d
